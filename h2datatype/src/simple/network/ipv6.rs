...
mod tests {
    ...
    fn test_ipv6() -> SimpleResult<()> {
        ...
        assert_eq!("::", IPv6::new(Endian::Big).to_display(context)?);

        let data = b"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f".to_vec();
        let context = Context::new(&data);

        assert_eq!("1:203:405:607:809:a0b:c0d:e0f", IPv6::new(Endian::Big).to_display(context)?);
    ...
    fn test_ipv6_little() -> SimpleResult<()> {
        ...
        assert_eq!("::1", IPv6::new(Endian::Little).to_display(context)?);
    ...
    fn test_ipv6_error() -> SimpleResult<()> {
        ...
        assert!(IPv6::new(Endian::Big).to_display(context).is_err());
