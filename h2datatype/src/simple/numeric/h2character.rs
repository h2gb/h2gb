...
mod tests {
    ...
    fn test_ascii() -> SimpleResult<()> {
        ...
        assert_eq!("'A'",     H2Character::new_ascii().to_display(Context::new_at(&data, 0), &DataNg::default())?);
        assert_eq!("'B'",     H2Character::new_ascii().to_display(Context::new_at(&data, 1), &DataNg::default())?);
        assert_eq!(format!("'{}'", 0xFF as char), H2Character::new_ascii().to_display(Context::new_at(&data, 2), &DataNg::default())?);
    ...
    fn test_utf8() -> SimpleResult<()> {
        ...
        assert_eq!("'A'", H2Character::new_utf8().to_display(Context::new_at(&data, 0), &DataNg::default())?);
        assert_eq!(1,     H2Character::new_utf8().base_size(Context::new_at(&data, 0))?);

        assert_eq!("'B'", H2Character::new_utf8().to_display(Context::new_at(&data, 1), &DataNg::default())?);
        assert_eq!(1,     H2Character::new_utf8().base_size(Context::new_at(&data, 1))?);

        assert_eq!("'❄'", H2Character::new_utf8().to_display(Context::new_at(&data, 2), &DataNg::default())?);
