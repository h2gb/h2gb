...
impl H2TypeTrait for H2Array {
    ...
    fn to_display(&self, context: Context, _data: &DataNg) -> SimpleResult<String> {
        ...
        let strings: Vec<String> = self.children_with_range(context)?.iter().map(|(range, _name, child)| {
            child.to_display(context.at(range.start), &DataNg::default())
...
mod tests {
    ...
    fn test_array_type() -> SimpleResult<()> {
        ...
        assert_eq!("[ 'A', 'B', 'C', 'D' ]", a.to_display(context, &DataNg::default())?);
        assert_eq!(0, a.related(context)?.len());
        assert_eq!(4, a.children(context)?.len());

        // Check the resolved version
        let r = a.resolve(context, None, &DataNg::default())?;
    ...
    fn test_dynamic_utf8_array() -> SimpleResult<()> {
        ...
        assert_eq!("[ 'A', 'B', '❄', '☢', '𝄞', '😈', '÷' ]", a.to_display(context, &DataNg::default())?);
