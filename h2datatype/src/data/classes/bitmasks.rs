...
impl Bitmasks {
    ...
    pub fn get_by_value(&self, value: &Integer, options: &BitmaskOptions) -> Vec<String> {
...
mod tests {
    ...
    fn test_csv() -> SimpleResult<()> {
        ...
        let flags = bitmasks.get_by_value(&Integer::from(1u32), &BitmaskOptions::new(None, false));
        assert_eq!(vec!["TEST1".to_string()], flags);

        // Test 0101 => 5
        let flags = bitmasks.get_by_value(&Integer::from(5u32), &BitmaskOptions::new(None, false));
        assert_eq!(vec!["TEST1".to_string(), "TEST2".to_string()], flags);

        // Test 0111 => 7 - no unknown_renderer set
        let flags = bitmasks.get_by_value(&Integer::from(7u32), &BitmaskOptions::new(None, false));
        assert_eq!(vec!["TEST1".to_string(), "TEST2".to_string()], flags);

        // Test 0111 => 7 - unknown_renderer set
        let renderer = ("Unknown_".to_string(), HexFormatter::new_integer(false, true, false));
        let flags = bitmasks.get_by_value(&Integer::from(7u32), &BitmaskOptions::new(Some(renderer), false));
...
TEST3: 5";
    ...
    fn test_show_negative() -> SimpleResult<()> {
        ...
        // Test the simple way
        let mut out = bitmasks.get_by_value(&Integer::from(7), &BitmaskOptions::new(None, false));
        out.sort();
        assert_eq!(vec!["TEST0", "TEST1", "TEST2"], out);

        let mut out = bitmasks.get_by_value(&Integer::from(7), &BitmaskOptions::new(None, true));
        out.sort();
