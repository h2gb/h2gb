...
use crate::datatype::{helpers, StaticType, ResolvedType, PartiallyResolvedType};
...
impl From<H2Simple> for StaticType {
    fn from(o: H2Simple) -> StaticType {
        StaticType::from(StaticType::H2Simple(o))
    }
}
...
impl H2Simple {
    ...
    pub fn partially_resolve(&self, start: u64) -> Vec<PartiallyResolvedType> {
        ...
        vec![PartiallyResolvedType {
            ...
            field_name: None,
            field_type: StaticType::from(self.clone()),
        }]
    }
...
mod tests {
    ...
    fn test_simple() -> SimpleResult<()> {
        ...
        let t: StaticType = H2Number::new(SizedDefinition::U32(Endian::Big), SizedDisplay::Hex(Default::default())).into();
        assert_eq!(4, t.size());
    ...
    fn test_simple_aligned() -> SimpleResult<()> {
        ...
        let t: StaticType = H2Simple::new_aligned(4,
